...
import type { BlogSectionConfig } from "@/lib/types"
...
interface BlogSectionProps {
  content: BlogSectionConfig
  items: BlogSectionItem[]
}

export function BlogSection({ content, items }: BlogSectionProps) {
      ...
      <div className="max-w-[1000px] mx-auto px-6 lg:px-12">
        ...
        <div className="space-y-0">
          {items.map((item, index) => (
            ...
            >
              <div>
                <p className="text-white/50 text-xs mb-2 group-hover:text-gold/70 transition-colors">
                  {item.date}
                </p>
        ...
        <div
          ...
          style={{ animationDelay: `${200 + items.length * 100}ms` }}
        >
          <Link
